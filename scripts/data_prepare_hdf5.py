...
def load_segmentation_tsv(tsv_file):
    ...
    """
    try:
        df = pd.read_csv(tsv_file, sep='\s+')
        
        # Group by read_id
        grouped = df.groupby('read_id')
        
        segmentation_data = {}
        for read_id, data in grouped:
            # Sort by kmer_idx
            data = data.sort_values('kmer_idx')
            
            segmentation_data[read_id] = {
                'start': np.array(data['start_raw_idx']),
                'end': np.array(data['end_raw_idx'])
            }
...
def process_batch(read_files, segmentation_file, output_file, window_length, overlap):
    ...
    segmentation_data = load_segmentation_tsv(segmentation_file)
...
def main(fast5_dir, segmentation_file, output_dir, total_files, 
         window_length, window_overlap, n_cores=1, verbose=True):
    ...
    Args:
        ...
        total_files (int): Number of output files to create
        window_length (int): Size of the chunks in raw datapoints
    ...
    file_batches = np.array_split(files_list, total_files)
    ...
    for i, batch in enumerate(file_batches):
        ...
        jobs.append(pool.apply_async(
            process_batch, 
            (batch.tolist(), segmentation_file, output_file, window_length, window_overlap)
        ))
    
    if verbose:
        print(f'Processing {len(jobs)} batches')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--total-files', type=int, default=10, help='Number of output files to create')
